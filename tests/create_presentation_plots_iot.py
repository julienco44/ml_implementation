"""
Create Presentation Visualizations for IOT Temperature Dataset
Run: python create_presentation_plots_iot.py
"""
...
OUTPUT_DIR = Path(__file__).parent / 'results' / 'figures_iot'
...
def load_data():
    """Load IOT Temperature dataset."""
    X_train = np.load(DATA_DIR / 'iot_temp_X_train.npy')
    X_test = np.load(DATA_DIR / 'iot_temp_X_test.npy')
    y_train = np.load(DATA_DIR / 'iot_temp_y_train.npy')
    y_test = np.load(DATA_DIR / 'iot_temp_y_test.npy')

    print(f"IOT Temperature Dataset loaded:")
...
def create_n_estimators_impact(n_estimators_values, train_r2, test_r2):
    ...
    ax.set_title('Impact of Ensemble Size (IOT Temperature)', fontweight='bold', pad=20)
    ax.legend()
    ax.grid(alpha=0.3)
...
def create_max_depth_impact(max_depth_values, train_r2, test_r2):
    ...
    ax1.set_title('Bias-Variance Trade-off: max_depth (IOT Temperature)', fontweight='bold', pad=20)
...
def create_ccp_tradeoff(ccp_alphas, test_r2, n_leaves):
    ...
    ax1.set_title('Cost-Complexity Pruning Trade-off (IOT Temperature)', fontweight='bold', pad=20)
    lines = [line1, line2]
...
if __name__ == "__main__":
    print("=" * 60)
    print("Creating Presentation Visualizations - IOT Temperature")
    print("=" * 60)
